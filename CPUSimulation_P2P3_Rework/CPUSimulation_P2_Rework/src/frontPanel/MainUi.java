...
import companent.CardReader;
import frontPanel.Loader;
import companent.Memory;

/**
 * you zhao yuma yufei zhou
 */
...
public class MainUi extends JFrame implements ActionListener, Observer {
	...
	public MainUi() {
		...
		// Create a framethis
		myMainFrame myMainFrame = new myMainFrame(500, 200, 990, 800,
				Color.WHITE);
		...
		keyboardPanel.setBounds(560,200, 394, 194);
	...
	public void actionPerformed(ActionEvent ae) {
		...
		case("Reset"):
			...
			displayPanel.getTextPane().setText("");
			printerPanel.getTextPane().setText("");
			keyboardPanel.RealFlag=false;
			keyboardPanel.RealKeyboard.setBackground(Color.BLUE);
			try {
				((CardReader)IODeviceDriver.getInstance().getCardReader(3)).reset();
				((CardReader)IODeviceDriver.getInstance().getCardReader(4)).reset();
